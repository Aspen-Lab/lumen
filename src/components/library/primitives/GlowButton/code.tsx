...
function resolveVariantColors(
  ...
  if (variant === "danger") {
    const f = DANGER_FROM;
    return {
      background:    \`linear-gradient(135deg, \${f} 0%, \${DANGER_TO} 100%)\`,
      border:        rgba(f, 0.6),
...
function Spinner({ color, sizeClass }: { color: string; sizeClass: string }) {
  return (
    <motion.span
      ...
      style={{ borderTopColor: color, borderRightColor: rgba("#FFFFFF", 0.2) }}
      animate={{ rotate: 360 }}
...
export function GlowButton({
  ...
  return (
    <motion.button
      ...
      whileHover={!isInert ? { scale: 1.01, y: -1, boxShadow: colors.hoverShadow } : {}}
    ...
    >
      ...
      <motion.span
        ...
        animate={!isInert ? { opacity: [0.4, 0.7, 0.4] } : { opacity: 0 }}
        transition={{ duration: 2.4, repeat: Infinity, ease: "easeInOut" }}
        style={{ background: colors.ambientGlow }}
      />

      {/* Content */}
      <span className="relative z-10 flex items-center w-full" style={{ gap: "inherit" }}>
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              <Spinner color={colors.textColor} sizeClass={s.spinnerSize} />
              <span
                className={\`font-semibold leading-tight \${s.labelSize}\`}
            ...
            >
              {icon && (
                <span
                  className={\`shrink-0 flex items-center justify-center \${s.iconSize}\`}
                  style={{ color: colors.textColor }}
                >
                  {icon}
                </span>
              ...
              <span className="flex flex-col items-start gap-0.5">
                ...
                {sublabel && (
                  <span
                    className={\`font-normal leading-tight \${s.sublabelSize}\`}
